...
// Snapshot payloads may contain arbitrary project files named *.asar. Use
// Electron's raw filesystem API so they remain opaque files during hashing,
// verification, copying, and rollback.
...
const PROJECT_RESTORE_MARKER = ".codex-link-project-restore-in-progress";

function cleanupRecordedProjectCopies(journal) {
  for (const target of journal?.projectFileTargets || []) {
    if (typeof target !== "string" || !path.isAbsolute(target)) continue;
    if (exists(path.join(target, PROJECT_RESTORE_MARKER))) removePath(target);
  }
}
...
function recoverInterruptedRestores(cloudDir) {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    try {
      ...
      cleanupRecordedProjectCopies(journal);
...
function executeRestoreTransaction({
  ...
  const projectFileTargets = [];
  const projectCopies = (plan.projectFileCopies || []).map((item) => {
    const source = path.resolve(item.source || "");
    const target = path.resolve(item.target || "");
    if (!isInside(path.join(plan.snapshotDir, "payload", "project-files"), source)
      || target === targetRoot || isInside(targetRoot, target)
      || target === path.resolve(plan.snapshotDir) || isInside(plan.snapshotDir, target)) {
      throw new Error(`Invalid project file restore path: ${item.label || target}`);
    }
    if (!statSafe(source)?.isDirectory() || exists(target)) {
      throw new Error(`项目文件目标必须不存在，且备份来源必须可用：${target}`);
    }
  ...
  try {
    ...
    for (const project of projectCopies) {
      projectFileTargets.push(project.target);
      updateTransactionJournal(rollback, "copying_project_files", {
        projectFileTargets,
        projectFileCopies: projectCopies.map(({ source, target, projectId }) => ({ source, target, projectId }))
      });
      fs.mkdirSync(project.target, { recursive: true });
      fs.writeFileSync(path.join(project.target, PROJECT_RESTORE_MARKER), "in progress\n", "utf8");
      copyPath(project.source, project.target);
      const issues = comparePaths(project.source, project.target, { allowUnexpected: true });
      if (issues.length) throw new RestoreExecutionError(`项目文件校验失败：${project.label}`, { phase: "project_files", issues: issues.slice(0, 100) });
    }
    ...
    for (const target of projectFileTargets) fs.rmSync(path.join(target, PROJECT_RESTORE_MARKER), { force: true });
    ...
    removePath(stageRoot);
    for (const target of projectFileTargets) {
      if (exists(path.join(target, PROJECT_RESTORE_MARKER))) removePath(target);
    }
...
function undoRestoreTransaction({ cloudDir, rollbackDir, id, processes, _testFailAfterItems = 0 }) {
  ...
  const projectCopies = Array.isArray(journal.projectFileCopies) ? journal.projectFileCopies : [];
  const projectFilesRetained = projectCopies.filter((project) => !exists(project.source) && exists(project.target)).map((project) => project.target);
  for (const project of projectCopies) {
    if (!project?.source || !project?.target || !exists(project.source)) continue;
    if (!exists(project.target) || comparePaths(project.source, project.target).length) {
      conflicts.push({ path: project.target, operation: "project_files", reason: "changed_after_restore" });
    }
  }
  if (conflicts.length) {
    updateTransactionJournal(rollback, "completed", {
  ...
  try {
    for (const project of projectCopies) {
      if (!exists(project.source) || !exists(project.target)) continue;
      const aside = `${project.target}.codex-link-undo-${crypto.randomBytes(4).toString("hex")}`;
      fs.renameSync(project.target, aside);
      projectMoves.push({ target: project.target, aside });
    }
    result = restoreFromRollback(rollback, { failAfterItems: _testFailAfterItems });
    if (!result.verified) {
      safetyRecovery = restoreUndoSafety(safety);
      for (const move of projectMoves.reverse()) fs.renameSync(move.aside, move.target);
    }
  } catch (error) {
    safetyRecovery = restoreUndoSafety(safety);
    for (const move of projectMoves.reverse()) {
      if (exists(move.aside) && !exists(move.target)) fs.renameSync(move.aside, move.target);
    }
    result = {
  ...
  const projectCleanupIssues = [];
  if (completed) for (const move of projectMoves) {
    try { removePath(move.aside); }
    catch (error) { projectCleanupIssues.push({ path: move.aside, error: error.message }); }
  }
