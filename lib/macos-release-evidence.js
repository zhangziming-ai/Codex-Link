...
function validateMacosReleaseEvidence(reportPath, expectedVersion) {
  ...
  try {
    ...
    if (validation.extractedZipLaunchAttempted !== true) failures.push("ZIP 解压应用未完成启动尝试");
    if (report.releaseMode === true && (validation.extractedZipLaunchSeconds < 8 || validation.extractedZipStayedAlive !== true)) failures.push("ZIP 解压应用未持续启动 8 秒");
    ...
    return {
      ...
      evidence: failures.length
        ? failures.join("；")
        : `Apple Silicon 实机通过：macOS ${report.host.version}，DMG/ZIP arm64，ZIP 已完成启动尝试${validation.extractedZipStayedAlive ? `并持续 ${validation.extractedZipLaunchSeconds} 秒` : ""}${report.releaseMode ? "，签名与公证通过" : "，内部测试模式"}`
    };
  } catch (error) {
