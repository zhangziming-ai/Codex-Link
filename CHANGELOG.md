...
- 项目文件可以逐个勾选备份，也可以全选当前电脑上存在的项目目录；旧电脑盘符对应的缺失目录会跳过文件复制，对话仍可备份。
- 恢复项目文件时可统一选择目标根目录，各项目仍保留独立文件夹，并更新恢复后项目与对话的路径关联。
- 修复 Electron 读取项目目录内 `app.asar` 时导致备份失败的问题。
- 更新备份与恢复界面，保留 SHA-256 完整性检查和恢复回滚。
- 已知问题：先前失败留下的无清单目录可能在备份管理中显示为 0.00 MB 的恢复点；该目录不能用于恢复。
